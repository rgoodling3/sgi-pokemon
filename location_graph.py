        ...
        for x in version:
            ...
            if v1 == game:
                ...
                for place in loc:
                    ...
                    place = change_location_name(place)
                    places.add(place)
...
def change_location_name(place):

    if (re.search(r"rt\|(\d*?)\|Kanto", place)):
        number = re.findall(r"rt\|(\d*?)\|Kanto", place)[0]
        place = place.replace(place, f"Kanto Route {number}")
    elif (place == 'In-game trade#Yellow|Trade'):
        place = "Trade for"
    elif ("List" in place):
        place = "Special Event"
    elif (place == 'Fishing#In the games|Super Rod'):
        place = "Fishing"
    elif (place == 'tt|*|immediately evolves into Machamp, but Pokédex data is entered'):
        place = "Immediately evolves"
    elif (place == "Diglett\\\\'s Cave"):
        place = "Diglett's Cave"
    elif ("|" in place):
        place = place.replace("|", " ")
